**About Me**


I am Hemachandra HD, a Vice President and Senior Scrum Master at Truist Bank with over 20 years of experience in the IT industry. Currently, I am pursuing a Master of Science in Data Science at Bellevue University, specializing in Generative AI and Financial Analytics.

My portfolio demonstrates my ability to solve complex business problems in the banking and finance sectors—ranging from AML Transaction Fraud Detection to Financial Sentiment Analysis—using state-of-the-art machine learning and Generative AI frameworks


## Work/Projects

### AML & Transaction Fraud Detection
* **Description**: An XGBoost-based classification model utilizing SMOTE and engineered "balance error" features to detect fraud in 6.3M mobile money transactions.
* **Technologies**: Python, XGBoost, SMOTE.

### Financial Sentiment Analysis (GenAI)
* **Description**: A specialized GAI application that processes earnings transcripts to provide domain-specific sentiment scores and reasoning. 
* **Approach**: Full fine-tuning of GPT-4o-mini to resolve "contextual reversals" in financial terminology.
* **Technologies**: OpenAI API, Python, Streamlit.

### Predictive Modeling for Banking Customer Churn
* **Description**: A comparative analysis using Logistic Regression and Random Forest to identify at-risk banking clients based on transaction frequency.
* **Technologies**: Python, Scikit-learn.

### Agile Metrics Dashboard
* **Description**: A professional dashboard covering Agile Release Train (ART) metrics to track sprint velocity, burn-down accuracy, and team efficiency.
* **Technologies**: Tableau/PowerBI, SQL.
